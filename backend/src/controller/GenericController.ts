...
import Cliente from '../domain/Cliente'; // Exemplo
import Endereco from '../domain/Endereco'; // Exemplo
import CartaoCredito from '../domain/CartaoCredito'; // Exemplo

// Instância única da Facade para ser usada pelo controller
...
export default class GenericController {
    ...
    async salvar(req: Request, res: Response) {
        try {
            // A lógica para criar a entidade a partir do req.body virá aqui
            // Por enquanto, um exemplo mockado
    ...
    async listar(req: Request, res: Response) {
        try {
            // Extrai o nome da entidade da URL, ex: /clientes -> Clientes
            const path = req.path.split('/')[1];
            const nomeEntidade = path.charAt(0).toUpperCase() + path.slice(1, -1);
            
            // Verifica se é uma listagem de sub-entidade (ex: /api/clientes/1/enderecos)
            const idRelacional = req.params.id ? parseInt(req.params.id) : undefined;
            const subEntidadePath = req.path.split('/')[3];

            if(subEntidadePath) {
                const nomeSubEntidade = subEntidadePath.charAt(0).toUpperCase() + subEntidadePath.slice(1, -1);
    ...
    async salvarEndereco(req: Request, res: Response) {
        try {
            ...
            const endereco = new Endereco(req.body.logradouro, req.body.numero, req.body.bairro, req.body.cidade, req.body.estado, req.body.cep, req.body.tipo);
    ...
    async alterarEndereco(req: Request, res: Response) {
        try {
            const endereco = new Endereco(req.body.logradouro, req.body.numero, req.body.bairro, req.body.cidade, req.body.estado, req.body.cep, req.body.tipo, parseInt(req.params.id));
    ...
    async salvarCartao(req: Request, res: Response) {
        try {
            const clienteId = parseInt(req.params.id);
            const cartao = new CartaoCredito(req.body.numero, req.body.nomeTitular, req.body.dataValidade, req.body.cvv);
    ...
    async alterarCartao(req: Request, res: Response) {
        try {
            const cartao = new CartaoCredito(req.body.numero, req.body.nomeTitular, req.body.dataValidade, req.body.cvv, parseInt(req.params.id));
    ...
    // Outros métodos (alterar, excluir, consultar) virão aqui...
} 